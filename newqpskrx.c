...
static void putbitbatch(void *state, unsigned data)
{
	...
	//printf("putbitbatch data %d (%04x) len %d shreg %08x\n", data, data, s->fec.bitbatchlen, s->shreg);
	if (data && !s->shreg)
	        s->shreg = 0x100;
...
static void rxidle(void *state)
{
	...
	for (i = 0; i < TuneCarriers; i++)
		s->tunephase[i] = _carg(s->tunecorr[i]);
...
static void rxtune(void *state)
{
	...
	for (i = 0; i < TuneCarriers; i++) {
		...
		s->syncdelay[i] = _carg(z);

		z.re = -cor2[i].re;
		z.im = cor2[i].im;
		s->syncphase[i] = _carg(z);
	}
...
{
	struct rxstate *s = (struct rxstate *)state;
	int rxword[SymbolBits];
	unsigned int errword[SymbolBits];
